...
const ImageCarouselModal = ({ isOpen, onClose, images, initialIndex = 0 }) => {
  ...
  useEffect(() => {
    // Reset to initialIndex when isOpen changes to true (modal opens)
    if (isOpen) {
  ...
  }, [isOpen, currentIndex, images.length]); // Add dependencies
          ...
          <div className="flex space-x-2 justify-center">
            {images.map((img, index) => (
              <button
                ...
                className={`w-16 h-16 md:w-20 md:h-20 rounded overflow-hidden shrink-0 focus:outline-none focus:ring-2 focus:ring-blue-500 ${
                  index === currentIndex ? 'ring-2 ring-blue-500 ring-offset-2' : 'opacity-70 hover:opacity-100'
                }`}
